...
Pagarme.prototype.request = function(options) {
  ...
  var requestParams = generateQuery({ api_key: this.key, format: 'json' }, options.query);
  return needle
    .requestAsync(options.method, this.url(options), requestParams, { timeout: 5000 })
    .bind(this)
    .spread(function(res, body) {
      if (res.statusCode !== 202 && res.statusCode < 400) return body;
      var message = body.error ? body.error.message : http.STATUS_CODES[res.statusCode] || 'Unknown';
      throw _.extend(new this.PagarmeError(message), {
